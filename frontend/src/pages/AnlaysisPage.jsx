...
export default function  AnalysisPage({ result, onBack }) {
  ...
  // Extract validation score from document validation agent text
  ...
  const validationScore = extractScore(agents.document_validation);
  const riskScore = extractRiskScore(agents.response_composer);
  const insightsScore = agents.key_insights?.includes("HIGH") ? 35 : agents.key_insights?.includes("MEDIUM") ? 60 : 80;
  const citationsScore = result.agents?.citations?.includes("HIGH") ? 30 : 70;
  const summaryScore = (() => {
    if (!agents.executive_summary || typeof agents.executive_summary === "object") return 65;
    return agents.executive_summary.includes("OVERVIEW") ? 45 : agents.executive_summary.includes("SUMMARY") ? 80 : 60;
  })();

  const radarData = [
    { metric: "Validation", score: validationScore },
    { metric: "Insights", score: insightsScore },
    { metric: "Summary", score: summaryScore },
    { metric: "Citations", score: citationsScore },
    { metric: "Low Risk", score: 100 - riskScore },
  ];

  const scoreCards = [
    { label: "Validation", score: validationScore, color: validationScore > 75 ? "#4CAF50" : validationScore > 50 ? "#FF9800" : "#f44336" },
    { label: "Insights", score: insightsScore, color: insightsScore > 75 ? "#4CAF50" : insightsScore > 50 ? "#FF9800" : "#f44336" },
    { label: "Summary Coverage", score: summaryScore, color: summaryScore > 75 ? "#4CAF50" : summaryScore > 50 ? "#FF9800" : "#f44336" },
    { label: "Citations", score: citationsScore, color: citationsScore > 75 ? "#4CAF50" : citationsScore > 50 ? "#FF9800" : "#f44336" },
    { label: "Overall Risk", score: 100 - riskScore, color: riskScore < 40 ? "#4CAF50" : riskScore < 70 ? "#FF9800" : "#f44336" },
  ];

  const agentColors = {
    executive_summary: "#2196F3", key_insights: "#4CAF50",
    citations: "#FF9800", document_validation: "#9C27B0",
    highlights: "#f44336", response_composer: "#1a1a2e"
  };

  const agentIcons = {
    executive_summary: "📝", key_insights: "💡",
    citations: "🔖", document_validation: "✅",
    highlights: "✨", response_composer: "🔍"
  };
      ...
      <div style={{ background: "white", padding: "1rem 2rem",
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "1rem" }}>
          ...
          <span style={{ fontWeight: "800", fontSize: "1.2rem" }}>
            DocIntel<span style={{ color: "#e94560" }}>Ai</span>
          </span>
        </div>
      ...
      <div style={{ padding: "2rem", maxWidth: "1100px", margin: "0 auto" }}>
        {tab === "simple" && (
          <>
            ...
            <div style={{ background: "white", borderRadius: "12px", padding: "1.5rem",
              boxShadow: "0 2px 8px rgba(0,0,0,0.06)" }}>
              <h3 style={{ margin: "0 0 1rem", color: "#1a1a2e" }}>🔍 Document Intelligence Summary</h3>
              <p style={{ color: "#444", lineHeight: "1.7", whiteSpace: "pre-wrap" }}>
                {typeof agents.response_composer === "string"
                  ? agents.response_composer.slice(0, 800) + "..."
            ...
            {Object.entries(agents).map(([agent, output]) => (
              <details key={agent} style={{ marginBottom: "1rem", background: "white",
                ...
                <summary style={{
                  ...
                  {agentIcons[agent]} {agent.split("_").map(part => part.charAt(0).toUpperCase() + part.slice(1)).join(" ")} Agent
                </summary>
